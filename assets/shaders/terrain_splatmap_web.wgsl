...
/// Altitude thresholds are tuned for HEIGHT_SCALE=150:
///   - Below 30% (45m): grass on flat, rock on steep
///   - 30%-70% (45m-105m): sand transition zone on moderate slopes
///   - Above 70% (105m): snow on flat, rock on steep
fn compute_splat_weights(normal_y: f32, world_y: f32, max_h: f32) -> vec4<f32> {
    ...
    let ALT_SNOW_START:  f32 = 0.90;
    let ALT_GRASS_END:   f32 = 0.60;
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Subtle distance fog (helps show depth on flat terrain)
